...
import { formatScore, getGrade, parseTFAnswer } from '../services/scoringService';
...
const TrueFalseDetail: React.FC<{
  ...
  // ✅ FIX: parser dùng chung — hiểu format mới "a:T,b:F" lẫn format cũ
  const tfMap = parseTFAnswer(userAnswer);
  const userAnswers: { [key: string]: boolean } = {};
  for (const [letter, val] of Object.entries(tfMap)) {
    userAnswers[letter] = val === 'T';
  }
